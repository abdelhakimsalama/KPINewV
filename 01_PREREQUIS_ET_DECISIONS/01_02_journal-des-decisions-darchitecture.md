...
**L'objectif reste le résultat juste, présenté simplement.** L'agent comprend la demande, interroge les données et rend le bon chiffre — directement, sans raconter comment il s'y est pris. Deux règles encadrent cela : il n'estime jamais et n'extrapole jamais depuis les lignes affichées (règle 6) ; et il donne le résultat, pas la méthode (règle 7). La vérifiabilité n'en souffre pas : le raisonnement de l'agent est consultable séparément dans la trace d'activité, et c'est là que le contrôle s'opère, pas dans la réponse à l'utilisateur.
