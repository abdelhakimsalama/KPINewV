...
| **Never promise completeness** (5 → 7) | Les règles reformulées 5 et 8 | L'agent réinvente des chiffres, comme le fait l'agent actuel avec ses « 202 champs renommés » |
| **Verbatim** (8 → 10) | La règle métier n°3 | Les libellés officiels sont traduits ou « corrigés », et deviennent inutilisables pour retrouver un champ dans SAC |
| **Never choose for the user** (11, 12) | La règle métier n°4 | L'agent tranche seul entre deux champs sans rapport, avec l'air d'être sûr |
| **Target Personas** (13, 14) | La règle métier n°6 | L'agent déduit un persona d'un intitulé de poste et filtre sur une hypothèse |
| **Language** (15 → 17) | La règle métier n°11 | Réponses dans la mauvaise langue, ou pire : libellés officiels traduits |
| **Answer format** (18 → 21) | Lisibilité et règles 12, 13 | Retour des tableaux Markdown et perte du signalement des anciens noms MyBI |
| **Conversation** (22, 23) | Ce que faisaient les topics `Greeting` et `Fallback` | Consommation inutile sur un « bonjour », et réponses hasardeuses hors périmètre |
...
2. **Règle 5 — ne jamais compter.** Sans moteur de calcul, tout chiffre serait une estimation présentée comme un fait.
3. **Règle 8 — verbatim.** Un libellé officiel modifié ne se retrouve plus dans SAC. La coquille fait partie de l'identifiant.
4. **Règle 11 — ne jamais choisir.** Deux champs peuvent répondre au même mot métier sans avoir le moindre rapport : `Mat: Product category` désigne un secteur d'activité (alcool, parfum, soin), `Fashion : Prod. Categ` un attribut mode (permanent, réassort, saisonnier). Trancher au hasard, c'est répondre faux une fois sur deux avec assurance.
...
| La conduite des questions multi-critères | Skill `sac-query-lookup` | Cas particulier des questions de requête |
